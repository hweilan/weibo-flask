...
def login():
    ...
    """
    form = LoginForm()
    if form.validate_on_submit():#validate判别类
        user = User.query.filter_by(email=form.email.data).first()#获取表单数据form.属性.data
        if user is not None and user.verify_password(form.password.data):
            login_user(user, form.remember_me.data)
            return redirect(request.args.get('next') or url_for('main.index'))
        flash('密码或账户错误')#闪现一条消息
...
def logout():
    ...
    """
    logout_user()
    #User.add_self_follow()加入自己关注自己
    flash('您已退出登录')
    return redirect(url_for('main.index'))#redirect 重定向
...
def register():
    ...
    if form.validate_on_submit():#判断非空
        ...
        send_mail(user.email,
                  ...
                  user=user,
                  token=token)
        # 自己关注自己，就可以在关注着微博列表中看到自己的微博，注册过的微博才能自己关注自己，生成的假数据用户不能
